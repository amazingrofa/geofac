...
import mpmath as mp
from mpmath import mpf, sqrt, ceil, floor, log
import random
...
def generate_close_prime_pair(base: int, min_gap: int, max_gap: int, seed: int) -> Tuple[int, int]:
    """
...
def run_experiment(
    ...
    for k_val, sign, scale, label in strategies:
        ...
        for i, sp in enumerate(semiprimes):
            result = run_single_test(
                sp, k_val, sign, scale, label, max_iterations
            )
            strategy_results.append(result)
            
            status = "✓" if result["success"] else "✗"
            print(f"    {i+1}. {status} n={sp['n']}: {result['iterations']} iterations")
        
        all_results.extend(strategy_results)
        
        # Strategy summary
        successes = sum(1 for r in strategy_results if r["success"])
        avg_iterations = sum(r["iterations"] for r in strategy_results) / len(strategy_results)
        print(f"  Summary: {successes}/{len(strategy_results)} successful, "
    ...
    experiment_results = {
        ...
        "strategies": [
            {"k": k, "sign": s, "scale": sc, "label": l}
            for k, s, sc, l in strategies
        ],
...
if __name__ == "__main__":
    ...
    # Save results
    output_path = "/home/runner/work/geofac/geofac/experiments/signed-scaled-adjustments/results.json"
